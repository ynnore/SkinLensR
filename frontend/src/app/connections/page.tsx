...
// Définir un type pour les données de connexion
...
const allTranslations = {
  connectionsPage: {
    ...
    communicationErrorPrefix: {
      ...
      'en-AU': 'Communication Error: ', 'en-NZ': 'Communication Error: ', 'en-CA': 'Communication Error: ', 'fr-CA': 'Erreur de communication : ', 'en-ZA': 'Kommunikasiefout: ', af: 'Kommunikasiefout: '
    },
    emptyMessage: {
...
export default function ConnectionsPage() {
  ...
  // Récupération des couleurs et variables CSS du thème
  ...
  // *** CORRECTION : ajout de warningText pour le CSS Module ***
  const warningText = theme === 'dark' ? '#FFCACA' : '#CC0000';

  // État pour gérer les données des connexions
  ...
  useEffect(() => {
    const fetchConnections = async () => {
      ...
      try {
        ...
        // Remplacez ceci par un appel réel à votre API backend.
        // Exemple: const response = await fetch('/api/connections');
        // if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        // const data: ConnectionNation[] = await response.json();
        // setNations(data);
        ...
        setError(err.message || getTranslation('connectionsPage', 'fetchErrorMessage', language));
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const formatLastActivity = (activityString: string): string => {
  ...
  return (
    <div
      ...
      style={{
        // Injecte les variables CSS pour le thème, utilisées par le module CSS
        ...
        '--kiwi-warning-text': warningText // Important pour les messages d'erreur
            ...
            <div key={nation.id} className={styles.nationCard}>
                  ...
                  style={{
                    backgroundColor:
                      nation.status === 'online'
                        ...
                        ? '#F44336' // Rouge
                        : '#FFC107', // Orange pour restricted
                  }}
                  title={getConnectionStatusTranslation(nation.status)}
                ></span>
              </div>
              <p className={styles.nationGroup}>
                {getTranslation('connectionsPage', 'groupLabel', language)} {nation.group}
              </p>
              ...
              <div className={styles.cardFooter}>
                <p style={{ color: mutedTextColor, fontSize: '0.85rem', fontStyle: 'italic', marginBottom: '0.5rem' }}>
                    {getTranslation('connectionsPage', 'lastActivityLabel', language)} {formatLastActivity(nation.lastActivity)}
                </p>
